...
//These functions are for the robots
String dirToText(int dir)
{
  switch(dir){
    case -1:
      return "Left";
...
/*
//return a random direction (for testing)
...
/******** BELOW THIS LINE IS ALL FUNCTIONS FOR THE GYRO UNIT *********/ 
...
boolean gyroBasedTurns(boolean dir, double angleDegrees) {
 ...
 while (abs(totalClicks) < abs(clicksToTurn)) {
  rawGyro = mpu.readRawGyro(); //read gyro values

  yawGyroValDouble =rawGyro.ZAxis;
...
/*outdated crap I can't be arsed to port
...
void getGyroValues(){
  ...
  byte rollGyroValMSB = readRegister(L3G4200D_Address, 0x43);
  byte rollGyroValLSB = readRegister(L3G4200D_Address, 0x44);
  rollGyroVal = ((rollGyroValMSB << 8) | rollGyroValLSB);

  byte pitchGyroValMSB = readRegister(L3G4200D_Address, 0x45);
  byte pitchGyroValLSB = readRegister(L3G4200D_Address, 0x46);
  pitchGyroVal = ((pitchGyroValMSB << 8) | pitchGyroValLSB);

  byte yawGyroValMSB = readRegister(L3G4200D_Address, 0x47);
  byte yawGyroValLSB = readRegister(L3G4200D_Address, 0x48);
